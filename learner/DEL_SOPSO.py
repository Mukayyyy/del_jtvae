...
class DEL_single():
    ...
    def train(self):
        ...
        for g in range(self.config.get('num_generations')):
            ...
            print('SO-PSO...')
            z = self.sopso(z, properties_single, w=0.9, c1=2, c2=2, steps=50)
    ...
    def sopso(self, z, score, w=0.9, c1=2, c2=2, steps=100):
        """
        Parameters
        ----------
        z : numpy array
        ...
        """
        N=z.shape[0] # number of particles
        dim = z.shape[1]
        # initialize
        z = z.astype('float32')
        particle_best = copy.deepcopy(z)
        ind = np.argmin(score)
        global_best = copy.deepcopy ( z[ ind ] )
        global_best_score = score[ind]
        v = 0.01* z #np.random.rand( N, dim )
       
        mlp = copy.deepcopy( self.trainer.model.mlp)    
               
        for s in range(steps):
            r1 = np.random.rand()
            r2 = np.random.rand()
            v = w*v + c1*r1 * (particle_best - z) + c2*r2*(global_best - z)
            z = z + v
            ...
            # if self.config.get('use_gpu'):
            #     zt = zt.cuda()
            #     #self.trainer.model.mlp.cuda()
            # with torch.no_grad():
            #     props = self.trainer.model.mlp( zt )
            ...
            # zvalid = z[valids]
            # # obtain fitness score / properties for generated samples
            # properties,propertiess_single = self.get_single_objective(new_samples, flip_qed_sign=True)
            # score[valid] = properties_single
           
            ind = score_new < score # get indices of better individual best
            particle_best[ind] = z[ind] # update particle best
            #score[ind] = score_new[ind] # update score
            ind = np.argmin(score_new)
            if score_new[ind] < global_best_score:
                global_best = copy.deepcopy ( z[ ind ] )
                global_best_score = score_new[ind]
