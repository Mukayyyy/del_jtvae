...
class JTNNVAE(nn.Module):
    ...
    def __init__(self, vocab, config):
        ...
        self.latent_size = latent_size = int(config.get('latent_size') / 2) #Tree and Mol has two vectors
        self.depthT = config.get('depthT')
        self.depthG = config.get('depthG')
        ...
        self.jtnn = JTNNEncoder(self.hidden_size, self.depthT, nn.Embedding(self.vocab.size(), self.hidden_size))
        self.decoder = JTNNDecoder(self.vocab, self.hidden_size, self.latent_size, nn.Embedding(vocab.size(), self.hidden_size))

        self.jtmpn = JTMPN(self.hidden_size, self.depthG)
        self.mpn = MPN(self.hidden_size, self.depthG)

        self.A_assm = nn.Linear(self.latent_size, self.hidden_size, bias=False)
        self.assm_loss = nn.CrossEntropyLoss(size_average=False)

        self.T_mean = nn.Linear(self.hidden_size, self.latent_size)  # nn2mean
        self.T_var = nn.Linear(self.hidden_size, self.latent_size)   # nn2logv
        self.G_mean = nn.Linear(self.hidden_size, self.latent_size)
        self.G_var = nn.Linear(self.hidden_size, self.latent_size)

        # self.embedder = nn.Embedding.from_pretrained(nn.Embedding(vocab.size(), hidden_size)) 
    ...
    # get sample vectors from latent space
    def sample_normal(self, dim, use_gpu=False):
        ...
        return z_tree + z_mol if use_gpu else z_tree, z_mol

    def sample_prior_z(self, z, prob_decode=False):
        # z_tree = torch.randn(1, self.latent_size).cuda()
        # z_mol = torch.randn(1, self.latent_size).cuda()
        return self.decode(z.cuda(), z.cuda(), prob_decode)
    ...
    def forward(self, x_batch, beta):
        ...
        # combine latent vector z_mol and z_tree
        z = torch.cat([z_tree_vecs, z_mol_vecs], dim=0).cuda()

        # getting mu and logvar
        ...
        mu = torch.cat([tree_mean, mol_mean], dim=0)
        logvar = torch.cat([tree_var, mol_var], dim=0)

        # predictor using combined latent z
        mlp_pred = self.mlp(torch.cat([z_tree_vecs, z_mol_vecs], dim=0))

        return word_loss + topo_loss + assm_loss + beta * kl_div, kl_div.item(), word_acc, topo_acc, assm_acc, z, mu, logvar, mlp_pred
